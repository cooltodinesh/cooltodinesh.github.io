...
getPromotionId("m").then(function (p) {
   promo_platform_id = getParameterByName('pid');

   if(promo_platform_id == 'myntra') {
    response = {
            "branding_info": {
        ...
        $.ajax({
            url: 'https://api.adcast.me/api/v1/end_users/campaign/'+promo_platform_id,
            type: 'get',
            headers: {
                sc: p
            },
...
function setPage() {
    // set theme
    branding_info = response['branding_info'];
    if (branding_info == null || typeof branding_info == undefined) {
        branding_info = {
            "text-primary": "color: #5c286e !important",
            "gradient-container": "background: linear-gradient(to bottom, antiquewhite, #79a2a078); background-attachment: fixed;",         
            "btn-primary": "background-color: #841d54; border-color: #841d54;",
    ...
    $(".text-primary").attr("style", branding_info["text-primary"]);
    $(".gradient-container").attr("style", branding_info["gradient-container"]);
    $(".btn-primary").attr("style", branding_info["btn-primary"]);
    $(".text-muted").attr("style", branding_info["text-muted"])
